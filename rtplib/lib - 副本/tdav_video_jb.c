	...
	if(!old_frame){
		...
		if(self->fps_prob <= 0 && self->avg_duration){
			...
			if(self->callback && (fps_old != self->fps)){
				...
				self->cb_data_any.ssrc = rtp_pkt->header->ssrc;
				self->cb_data_any.fps.new_ = self->fps; // clipped value
...
const trtp_rtp_packet_t* tdav_video_jb_pkt_find_by_seq(tdav_video_jb_t* self, unsigned  short seq)
{
	...
	const tsk_list_item_t *item;
	 ...
	 const trtp_rtp_packet_t* result = tsk_null;

	tsk_list_lock(self->frames);
	tsk_list_foreach(item, self->frames){
		if(TDAV_VIDEO_FRAME(item->data)->highest_seq_num >= seq){
			ret = item->data;
			break;
		}
	...
	if(ret) {
		result = tdav_video_frame_find_by_seq_num(ret, seq);		
	}
