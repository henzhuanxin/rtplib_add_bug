...
//printf("rtp_packet->payload.size=%d\n",rtp_packet->payload.size);
	// Packet
	...
	// For now, always single-level protection
	{
		...
		level0->hdr.mask |= (uint64_t)((uint64_t)1 << (level0->hdr.mask_size - (rtp_packet->header->seq_num - self->encoder.pkt->hdr.SN_base.value)-1));
...
const tsk_object_def_t *tdav_codec_ulpfec_def_t = &tdav_codec_ulpfec_def_s;
tdav_codec_ulpfec_t* tsk_fec_create()
{
	return tsk_object_new(tdav_codec_ulpfec_def_t);
}
